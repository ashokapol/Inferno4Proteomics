...
namespace DAnTE.Inferno
{
    public partial class frmDAnTEmdi : Form
    {
        ...
        public frmDAnTEmdi(string dntfile, string logfile)
        {
            ...
            this.Text = "Inferno " + "1.1"; //Application.ProductVersion.ToString();

            SplashScreen.ShowSplashScreen();
            Application.DoEvents();

            Log(mblLog, "Starting Inferno...", logwriter);
            SplashScreen.SetStatus("Starting Inferno...");
            ...
            tempFile = tempPath.Replace("\\", "/") + "/Inferno/_temp.png";
            ...
            if (!rConnector.initR())
            {
                startupErrString.Append("* R failed to initialize for some unknown reason. Check if statconnDCOM is working properly.").AppendLine();
                //SplashScreen.CloseForm();
        ...
        private void Log(bool mblog, String logMessage, TextWriter w)
        {
            if (mblog)
            {
                //w.Write("\r\nLog Entry : ");
