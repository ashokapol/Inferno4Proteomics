    ...
    partial class frmDAnTE
	{
        ...
        private void menuItemLoad_Click(object sender, System.EventArgs e)
		{
            ...
            if (mstrOriginator.Contains("Expression"))
            {
                ...
                else
                {
                    ...
                    GetOpenFileName("CSV files (*.csv)|*.csv|Tab delimited txt files (*.txt)|*.txt|" +
        ...
        private void mnuSaveSession_Click(object sender, EventArgs e)
        {
            if (mtabControlData.Controls.Count != 0)
            {
                ...
                try
                {
                    rConnector.rdcom.EvaluateNoReturn("dummy<-0"); // create some dummy variable
                    rConnector.rdcom.EvaluateNoReturn(rcmd1);
                }
                catch (Exception ex)
                {
                    Console.WriteLine("RDCOM failed: " + ex.Message, "Error!");
                    success = false;
                }
                if (success)
                {
