    ...
    partial class frmDAnTE
    {
        ...
        private void GetOpenFileName(string filter)
        {
            ...
            if (fdlg.ShowDialog() == DialogResult.OK)
            {
                ...
                Settings.Default.DataFileName = Path.GetFileName(mstrLoadedfileName);
        ...
        private DataTable LoadProtColumns(DataTable mDT, string protIPI, string rowID)
        {
            ...
            try
            {
                ...
                try
                {
                    rConnector.rdcom.SetSymbol("protIPI", mstrArrProteins);
                    rConnector.rdcom.SetSymbol("MassTags", mstrArrMassTags);
                    ...
                    if (rConnector.GetTableFromRProtInfoMatrix("ProtInfo"))
                    {
                        ...
                        mDTProts = rConnector.mDataTable.Copy();
                        mDTProts.TableName = "ProtInfo";
        ...
        private DataTable GetProtInfoMatrix()
        {
            ...
            {
                clsRarray.rowNamesID = "Row_ID";
                mDTProtInfo = rConnector.mDataTable.Copy();
                        ...
                        if (mfrmSelectCols.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            try
                            {
                                ...
                                if (mhtDatasets.ContainsKey("Factors"))
                                {
                                    ...
                                    rConnector.rdcom.EvaluateNoReturn(rcmd);
                                    FactorsValid = (bool)rConnector.rdcom.GetSymbol("FactorsValid");
                                }
                                if (!FactorsValid)
                                {
                                    success = false;
                                ...
                                if (success)
                                {
                                    ...
                                    rConnector.rdcom.EvaluateNoReturn("print(dim(Eset))");
                                    rConnector.rdcom.EvaluateNoReturn("cat(\"Expressions loaded.\n\")");
                                }
                            }
                            catch (Exception ex)
                            {
                            ...
                            // Duplicates are handled during the call 'SendTable2RmatrixNumeric'
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        mDTselectedEset1 = rConnector.mDataTable.Copy();
                                        mDTselectedEset1.TableName = "Eset";
                                        rConnector.rdcom.EvaluateNoReturn("print(dim(Eset))");
                                        rConnector.rdcom.EvaluateNoReturn("cat(\"Expressions loaded.\n\")");
                                        AddDataset2HashTable(mDTselectedEset1);
        ...
        private void UpdateFactorInfoArray()
        {
            ...
            try
            {
                rConnector.rdcom.EvaluateNoReturn("factorNames <- rownames(factors)");
                object factorNames = rConnector.rdcom.GetSymbol("factorNames");
                string[] factors = (string[])factorNames;
                for (int numF = 0; numF < factors.Length; numF++)
                {
                    marrFV.Clear();
                    rConnector.rdcom.EvaluateNoReturn("fVals <- unique(factors[" +
                        Convert.ToString(numF + 1) + ",])");
                    rConnector.rdcom.EvaluateNoReturn("nfVals <- length(fVals)");
                    object fVals = rConnector.rdcom.GetSymbol("fVals");
                    object nfVals = rConnector.rdcom.GetSymbol("nfVals");
                    if ((int)nfVals > 1) //more than one factor value
                    {
                        factorVs = (string[])fVals;
                        for (int i = 0; i < factorVs.Length; i++)
                            marrFV.Add(factorVs[i]);
                    }
                    else
                        marrFV.Add((string)fVals);
