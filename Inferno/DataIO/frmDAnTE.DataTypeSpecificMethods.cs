...
namespace DAnTE.Inferno
{
    ...
    partial class frmDAnTE
	{
        private void AddDataset2HashTable(DataTable mDT)
        {
            ...
            clsDatasetTreeNode mclsDataset = new clsDatasetTreeNode();
            mclsDataset.mDTable = mDT;
        ...
        private bool OpenSession(string filename)
        {
            ...
            try
            {
                ...
                vars = rConnector.rdcom.GetSymbol("vars");
                rConnector.rdcom.EvaluateNoReturn("print(vars)");
            }
            catch (Exception ex)
            {
                    ...
                    try
                    {
                        switch (variables[i])
                        {
                            ...
                            case ("factors"):
                                ...
                                {
                                    mDTvar = rConnector.mDataTable.Copy();
                            ...
                            case ("pData1"):
                                #region RRollup
                                rConnector.rdcom.EvaluateNoReturn("pData1 <- pScaled1$pData");
                                if (rConnector.GetTableFromRproteinMatrix("pData1"))
                                {
                                    mDTvar = rConnector.mDataTable.Copy();
                                    mDTvar.Columns[0].ColumnName = "Protein";
                                    rConnector.rdcom.EvaluateNoReturn("pData11 <- pData1[,-c(1,2)]");
                            ...
                            case ("pData2"):
                                #region ZRollup
                                rConnector.rdcom.EvaluateNoReturn("pData2 <- pScaled2$pData");
                                if (rConnector.GetTableFromRproteinMatrix("pData2"))
                                {
                                    mDTvar = rConnector.mDataTable.Copy();
                                    mDTvar.Columns[0].ColumnName = "Protein";
                                    rConnector.rdcom.EvaluateNoReturn("pData22 <- pData2[,-c(1,2)]");
                            ...
                            case ("qrollupP"):
                                ...
                                if (rConnector.GetTableFromRproteinMatrix("qrollupP"))
                                {
                                    mDTvar = rConnector.mDataTable.Copy();
                                    mDTvar.Columns[0].ColumnName = "Protein";
                                    rConnector.rdcom.EvaluateNoReturn("qrollupP1 <- qrollupP[,-c(1,2)]");
        ...
        private void AddDataNode(clsDatasetTreeNode mdataNode)
        {
            ...
            if ((mtabControlData.Controls.Count == 0) &&
                (mhtDatasets.ContainsKey("Expressions") || mhtDatasets.ContainsKey("Protein Info") || 
