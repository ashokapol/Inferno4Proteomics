...
namespace DAnTE.Inferno
{
    partial class frmDAnTE
	{
        ...
        private void ChangeDatasetOrderR(ArrayList marrNewOrder)
        {
            ...
            try
            {
                rConnector.rdcom.EvaluateNoReturn("ls()");
                ...
                if (mfrmFactorInfo.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (marrFactorInfo.Count > 0)
                    {
                        ...
                        if (rConnector.SendTable2RmatrixNonNumeric("factors", mDTFactors))
                        {
                            try
                            {
                                rConnector.rdcom.EvaluateNoReturn("print(factors)");
                                rConnector.rdcom.EvaluateNoReturn("cat(\"Factors loaded.\n\")");
                            }
                            catch (Exception ex)
                            {
                                MessageBox.Show("Error: " + ex.Message, "Exception while talking to R");
        ...
        private void mnuItemFC_Click(object sender, EventArgs e)
        {
                ...
                mclsSelected.mblIsNumeric)
            {
                ...
                if (mfrmFC.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        rConnector.rdcom.EvaluateNoReturn(mclsFoldChangePar.Rcmd);
                        if (rConnector.GetTableFromRmatrix("foldChanges"))
                        {
                            mDTFoldChange = rConnector.mDataTable.Copy();
            ...
            if (mclsSelected.mDTable != null)
            {
                ...
                if (mfrmRowFilter.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        {
                            mDTfiltered = rConnector.mDataTable.Copy();
                            if (mDTfiltered != null)
                            {
        ...
        private bool DoAnova(string rcmd)
        {
            ...
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("pvalues<-anovaR$pvals");
                rConnector.rdcom.EvaluateNoReturn("notused<-anovaR$miss");
                rConnector.rdcom.EvaluateNoReturn("allused<-anovaR$allused");
                mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");
            }
            catch (Exception ex)
            {
                MessageBox.Show("RDCOM failed: " + ex.Message, "Error!");
            ...
            {
                mDTanovaP = rConnector.mDataTable.Copy();
            ...
            if (success)
            {
                if (!mblAnovaAllUsed)
                {
                    ...
                    mDTanovaX = rConnector.mDataTable.Copy();
        ...
        private bool DoTamuQ(string rcmd)
        {
            ...
            /*bool mblAnovaAllUsed = false;*/
            bool success = true;
            //rConnector.rdcom.EvaluateNoReturn("a <- DoAnova");
            //rConnector.rdcom.EvaluateNoReturn("a <- DoTamuQ");
            //rConnector.rdcom.EvaluateNoReturn("pvalues<-1");
            //int a = 1;
            //rConnector.rdcom.EvaluateNoReturn("pvalues<-DoAnova");
            ...
            try
            {
              //  rConnector.rdcom.EvaluateNoReturn("pvalues<-DoAnova");
                ...
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("yimputed<-tamuQ$y");
                rConnector.rdcom.EvaluateNoReturn("pvalues<-tamuQ$pvals");
                //rConnector.rdcom.EvaluateNoReturn("notused<-TamuQ$miss");
                //rConnector.rdcom.EvaluateNoReturn("allused<-TamuQ$allused");
                /*mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");*/
            }
            catch (Exception ex)
            {
            ...
            {
                mDTamuQYImputed = rConnector.mDataTable.Copy();
            ...
            if (success && rConnector.GetTableFromRmatrix("pvalues"))
            {
                mDTamuQP = rConnector.mDataTable.Copy();
            ...
            if (success)
            {
                ...
                {
                    rConnector.GetTableFromRmatrix("notused");
                    mDTamuQX = rConnector.mDataTable.Copy();
        ...
        private bool DoKWtest(string rcmd)
        {
            ...
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("pvalues<-kwtest$pvals");
                rConnector.rdcom.EvaluateNoReturn("notused<-kwtest$miss");
                rConnector.rdcom.EvaluateNoReturn("allused<-kwtest$allused");
                mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");
            }
            catch (Exception ex)
            {
        ...
        private bool DoWilcoxtest(string rcmd)
        {
            ...
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("pvalues<-wilcoxtest$pvals");
                rConnector.rdcom.EvaluateNoReturn("notused<-wilcoxtest$miss");
                rConnector.rdcom.EvaluateNoReturn("allused<-wilcoxtest$allused");
                mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");
            }
            catch (Exception ex)
            {
        ...
        private bool DoShapiroWilkstest(string rcmd)
        {
            ...
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("pvalues<-swtest$pvals");
                rConnector.rdcom.EvaluateNoReturn("notused<-swtest$miss");
                rConnector.rdcom.EvaluateNoReturn("allused<-swtest$allused");
                mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");
            }
            catch (Exception ex)
            {
        ...
        private bool DoOneSampleTtest(string rcmd)
        {
            ...
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
                rConnector.rdcom.EvaluateNoReturn("pvalues<-ttest$pvals");
                rConnector.rdcom.EvaluateNoReturn("notused<-ttest$miss");
                rConnector.rdcom.EvaluateNoReturn("allused<-ttest$allused");
                mblAnovaAllUsed = (bool)rConnector.rdcom.GetSymbol("allused");
            }
            catch (Exception ex)
            {
