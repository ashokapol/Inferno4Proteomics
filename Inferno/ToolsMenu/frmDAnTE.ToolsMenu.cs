...
namespace DAnTE.Inferno
{
    partial class frmDAnTE
	{
            ...
            if (mhtDatasets.ContainsKey("p-Values"))
            {
                ...
                if (mfrmFilterANOVA.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        rConnector.rdcom.EvaluateNoReturn("err<-filterResult$error");
                        rConnector.rdcom.EvaluateNoReturn("nodata<-filterResult$NoData");
                        rConnector.rdcom.EvaluateNoReturn(filtTableName + "<-filterResult$Filtered");
                        mblErr = (bool)rConnector.rdcom.GetSymbol("err");
                        mblNoData = (bool)rConnector.rdcom.GetSymbol("nodata");
                    }
                    catch (Exception ex)
                    {
                ...
                if (mhtDatasets.ContainsKey("Expressions") && mclsSelected.mblIsPlottable)
                {
                    ...
                    if (mfrmMissing.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                mDTmissFilt = rConnector.mDataTable.Copy();
                                if (mDTmissFilt != null)
                                {
        ...
        private void mnuItemIntensityFilt_Click(object sender, EventArgs e)
        {
            if (ctltreeView.SelectedNode.Tag != null)
            {
                DataTable mDTmissFilt = new DataTable();
                ...
                if (mhtDatasets.ContainsKey("Expressions") && mclsSelected.mblIsPlottable)
                {
                    ...
                    frmIntensityFilter mfrmIntensFilter = new frmIntensityFilter();
                    mfrmIntensFilter.DataSetName = mclsSelected.mstrDataText;


                    if (mfrmIntensFilter.ShowDialog() == DialogResult.OK)
                    {
                        filterCutoff = mfrmIntensFilter.CutOff;
                        method = mfrmIntensFilter.Method;
                        mintFilterTblNum++;
                        filtTableName = "filteredData" + mintFilterTblNum.ToString();
                        rcmd = filtTableName + "<- intensityFilter(" + dataset + "," + filterCutoff + "," + method + ")";
                        try
                        {
                            rConnector.rdcom.EvaluateNoReturn(rcmd);
                            if (rConnector.GetTableFromRmatrix(filtTableName))
