...
namespace DAnTE.Inferno
{
	...
	{		
		...
		private ArrayList marrJobIDs = new ArrayList();
        private string[] mstrArrProjects;

        private Hashtable mhtJobIDs = new Hashtable();
        ...
        private Hashtable mhTable = new Hashtable();
        ...
        public ctlMSMSSelectFilesFromDBFilterWizPage()
		{
			...
			this.SetActive += new System.ComponentModel.CancelEventHandler(this.ctlSelectFilesFromDBFilterWizardPage_SetActive) ;
		}

        public ctlMSMSSelectFilesFromDBFilterWizPage(clsRconnect rdcom)
        {
            rConnector = rdcom;
        ...
        private bool FetchMatchingData()
        {
            ...
            string mstrProj = (string)mcmbBoxProjects.SelectedItem;
                ...
                //check if filter string has changed
            {
                ...
                if (GetJobs2Table(wildcardFilter, mstrUrl, mstrFolder, "ms2", "DatasetNames"))
                {
        ...
        private void AddToList(DataTable mDT)
        {
            foreach (DataRow drow in mDT.Rows)
            {
                ...
                {
                    mhTable.Add(drow[1].ToString(), string.Empty);
                    ListViewItem dataItem = new ListViewItem(drow[1].ToString());
        ...
        private bool GetLabkeyProjectNames()
        {
            bool success = true;
            string rcmd = null;
            rcmd = @"projectNames <- LabKeyProjects()";
            try
            {
		...
		private bool GetJobs2Table(string datasetname, string mstrURL, string mstrPath, string schema, string query)
		{
            bool success = true;
            string rcmd = null;
            rcmd = @"fileNames <- LabkeyFetch(""" + datasetname + @""",""" + mstrURL + @""",""/" + mstrPath + @"/"",""ms2"",""DatasetNames"")";
            try
            {
                rConnector.rdcom.EvaluateNoReturn(rcmd);
            }
            ...
            {
                mdtLabKeyFiles = rConnector.mDataTable.Copy();
		...
		{
			for (int i = 0; i < mlstViewJobs.Items.Count; i++)
			{
				if (mlstViewJobs.Items[i].Checked == true)
				{
